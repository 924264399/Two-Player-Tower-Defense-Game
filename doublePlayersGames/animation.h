...
public:
	...
	///核心 渲染的  输入渲染的位置  有的序列动画可能还需要旋转 所以这里给了一个angle角度的输入
	void on_render(SDL_Renderer* renderer, const SDL_Point& pos_dst, double angle = 0) const
	{
		...
		SDL_RenderCopyEx(renderer, texture, &rect_src_list[idx_frame], &rect_dst, angle, nullptr, SDL_RendererFlip::SDL_FLIP_NONE);  //第三个是裁剪矩形  rect_dst目标矩形  SDL_FLIP_NONE不进行翻转 
	}
