...
#include <unordered_map>   //这个是哈希表 就是字典  用于查找非常方便  这里用作资源的容器


//这个枚举是 涵盖了游戏内所有用到的资源的名称   给资源取名字
enum class ResID
{
	Tex_Tileset,   //瓦片图集的纹理

	Tex_Player,    //不同角色的序列帧图片
	Tex_Archer,
	Tex_Axeman,
	Tex_Gunner,
	...
	Tex_Goblin,
	Tex_GoblinPriest,
	Tex_SlimeSketch,     //带sketch 后缀的，是白色剪影图片
	Tex_KingSlimeSketch,
	Tex_SkeletonSketch,
	Tex_GoblinSketch,
	Tex_GoblinPriestSketch,

	Tex_BulletArrow,    //子弹类型  防御塔射的是arrow  弓箭手射的是箭  炮手射出的shell炮弹
	Tex_BulletAxe,
	Tex_BulletShell,
	...
	Sound_ArrowFire_1,      //音效   arrow发射    hit受击音效
	Sound_ArrowFire_2,
	Sound_AxeFire,
	Sound_ShellFire,
	Sound_ArrowHit_1,
...
public:
	typedef std::unordered_map<ResID, TTF_Font*> FontPool;   //存储字体放在 FontPool

	typedef std::unordered_map<ResID, Mix_Chunk*> SoundPool;   //存储音频

	typedef std::unordered_map<ResID, Mix_Music*> MusicPool;   //存储音乐
...
public:
	...
	bool load_from_file(SDL_Renderer* renderer)
	{
		...
		for (const auto& pair : texture_pool)  //texture_pool全部加载完成之后  就对其每个元素进行遍历  看看每个元素是不是有效（加载失败就是None）  
	...
	const FontPool& get_font_pool()    //通过这个在外部获取字体资源池  返回常量引用  外部调用者​​无法通过这个引用修改 font_pool对象​ 就只读
	{
		return font_pool;
	}
